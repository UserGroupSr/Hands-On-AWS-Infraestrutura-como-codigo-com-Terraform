...
dnf install -y nginx || error_exit "Falha ao instalar nginx"

# AWS CLI v2 já vem pré-instalado no Amazon Linux 2023
echo "Verificando AWS CLI..."
aws --version || error_exit "AWS CLI não encontrado"

#########################################################
# HABILITA E INICIA SSM AGENT (Systems Manager)
#########################################################
# No Amazon Linux 2023, o SSM Agent pode não vir pré-instalado
# dependendo da AMI utilizada. Instalamos se necessário.

echo "Habilitando e iniciando SSM Agent..."

if ! systemctl list-unit-files | grep -q "amazon-ssm-agent"; then
    echo "SSM Agent não encontrado, instalando..."
    dnf install -y amazon-ssm-agent || error_exit "Falha ao instalar SSM Agent"
fi

systemctl enable amazon-ssm-agent || error_exit "Falha ao habilitar SSM Agent"
systemctl start amazon-ssm-agent || error_exit "Falha ao iniciar SSM Agent"
...
if [ "$HTTP_STATUS" != "200" ]; then
    error_exit "Nginx não respondeu com 200"
fi
